...
<body>
	...
	<form id="sform" action="userManage.do" method="post" align="center"
		...
		<input type="hidden" name="markManage" id="markManage"
			...
			type="hidden" name="studioStatus" id="studioStatus"
			value="${user.studioStatus}" /><input type="hidden" name="condition"
			id="condition" value="${user.condition}" />
		<table align="center" height="90%">
			<tr height="13px">
				<td align="center">品牌名：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" placeholder="不能为空" id="dBrand" name="dBrand"
							style="margin-top: 0px;" value="${user.dBrand}"
							class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.dBrand}</c:if></td>
			</tr>
			<tr>
				<td>品牌logo：</td>
				<td><img id="imagez"
					src="<%=path%>/upload/uploadImg/user/${user.avatar}"
					style="width: 80px; height: 80px;" /> <c:if
						...
						<input type="hidden" name="oldImg" id="oldImg"
							value="${user.avatar}" disabled="disabled" />
					</c:if></td>
			</tr>
			<tr>
				<td>品牌标签：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" name="dTag" placeholder="多个请以中文、隔开"
							style="margin-top: 0px;" id="dTag" value="${user.dTag}"
							class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.dTag}</c:if></td>
			</tr>
			<tr>
				<td>品牌故事：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
				<textarea id="applyReason" name="applyReason">${user.applyReason}</textarea>
						 
					</c:if> <c:if test="${user.markManage==3}">${user.applyReason}</c:if></td>
			</tr>
			<tr>
				<td>品牌所在地：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" name="dAddress" style="margin-top: 0px;"
							id="dAddress" value="${user.dAddress}" class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.dAddress}</c:if></td>
			</tr>
			<tr height="13px">
				<td align="center">设计师名：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" placeholder="不能为空" id="dUsername"
							name="dUsername" style="margin-top: 0px;"
							value="${user.dUsername}" class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.dUsername}</c:if></td>
			</tr>
			<tr height="13px">
				<td align="center">用 户 名：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" placeholder="不能为空" id="username"
							name="username" onblur="checkView(this);"
							onfocus="getfocus(this);" style="margin-top: 0px;"
							value="${user.username}" class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.username}</c:if></td>
			</tr>
			<tr>
				<td>真实姓名：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" name="realname" style="margin-top: 0px;"
							id="realname" value="${user.realname}" class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.realname}</c:if></td>
			</tr>
			<tr>
				<td>手 机 号：</td>
				<td><c:if test="${user.markManage==1 || user.markManage==2}">
						<input type="text" name="phone" style="margin-top: 0px;"
							id="phone" value="${user.phone}" onblur="checkView(this)"
							onfocus="getfocus(this)" class="nav-search-input">
					</c:if> <c:if test="${user.markManage==3}">${user.phone}</c:if></td>
			</tr>
			<tr>
				<td colspan="2" align="center"><c:if
						test="${user.markManage==1 || user.markManage==2}">
						<input type="submit" value="保存" class="btn btn-primary btn-sm" />&nbsp;&nbsp;<input
							type="button" onclick="back(2)" value="返回"
							class="btn btn-primary btn-sm" />
					</c:if> <c:if test="${user.markManage==3}">
						<input type="button" onclick="back(2)" value="返回"
							class="btn btn-primary btn-sm" />
					 </c:if></td>
...
<script type="text/javascript">
	...
	function back(){
		...
		window.location.href="<%=request.getContextPath()%>/userManage/queryList.do?pageInfo.pageIndex="
				+ pageIndex + "&groupId=4&markUserfrom=" + markUserfrom+"&condition="+condition;
	}
	//上传头像
